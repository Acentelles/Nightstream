...
def _format_u8_array(name: str, base: int, content: bytes, sha256_hex: str) -> str:
    lines = []
    lines.append("// @generated by crates/deprecated-neo-fold/riscv-tests/guests/rv32-u64-output/export_rom_rs.py")
...
def main() -> int:
    ...
    out_rs = repo_root / "crates/deprecated-neo-fold/riscv-tests/binaries/rv32_u64_output_rom.rs"

    subprocess.run(["cargo", "build", "--release"], cwd=guest_dir, check=True)

    elf = guest_dir / "target/riscv32im-unknown-none-elf/release/rv32_u64_output"
    if not elf.exists():
    ...
    out_rs.write_text(_format_u8_array("RV32_U64_OUTPUT_ROM", base, text, sha256_hex), encoding="utf-8")
