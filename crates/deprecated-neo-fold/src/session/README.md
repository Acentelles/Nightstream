...
### 2) Shared CPU-bus R1CS circuits (Twist + generic lookup)

This is the ergonomic path for zkVM-like traces where Twist/generic-lookup semantics are proven via the
...
- Implement `NeoCircuit` to:
  ...
  - build the CPU witness prefix per chunk, typically:
    ...
    - call `TwistPort::fill_from_trace(...)` / `LookupPort::fill_from_trace(...)` to auto-fill CPU binding columns
