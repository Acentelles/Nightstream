...
pub(crate) fn prepare_ccs_for_shared_cpu_bus_steps<'a, Cmt, S: BusStepView<Cmt>>(
    ...
    let canonical_trace_mode = !steps.is_empty() && m_in == 5 && steps[0].time_cpu_cols_len() == rv64_trace_cpu_cols;
    ...
    if route_a_uniform_mode && !has_physical_bus_refs {
        // Only the canonical RV64 trace-wiring path is allowed to satisfy shared-bus
