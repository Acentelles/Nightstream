use deprecated_neo_memory::riscv::exec_table::{Rv32ExecRow, Rv32ExecTable, Rv32ShoutEventRow, Rv32ShoutEventTable};
...
fn rv32m_from_trace_rejects_tampered_virtual_micro_op_shape() {
    ...
    let read_lane0 = trace.steps[first_virtual_idx]
        ...
        .iter_mut()
        .find(|ev| {
            ev.twist_id == REG_ID && matches!(ev.kind, deprecated_neo_vm_trace::TwistOpKind::Read) && ev.lane == Some(0)
        })
