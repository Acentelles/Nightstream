...
use deprecated_neo_memory::riscv::trace::{extract_shout_lanes_over_time, extract_twist_lanes_over_time};
...
fn trace_sidecar_extract_rejects_multiple_shout_events() {
    ...
    let ev = deprecated_neo_vm_trace::ShoutEvent::<u128, u64> {
        shout_id: deprecated_neo_vm_trace::ShoutId(shout_table_ids[0]),
        key: 0,
        value: 0,
    };
...
fn trace_sidecar_extract_rejects_multiple_ram_writes() {
    ...
    let sw_row = exec
        ...
        .position(|r| {
            r.ram_events
                .iter()
                .any(|e| matches!(e.kind, deprecated_neo_vm_trace::TwistOpKind::Write))
        })
        .expect("expected a RAM write row");
    let write_ev = exec.rows[sw_row]
        .ram_events
        .iter()
        .find(|e| matches!(e.kind, deprecated_neo_vm_trace::TwistOpKind::Write))
        .cloned()
        .expect("write event");
...
fn trace_sidecar_extract_rejects_read_write_addr_mismatch() {
    ...
    read_ev.kind = deprecated_neo_vm_trace::TwistOpKind::Read;
