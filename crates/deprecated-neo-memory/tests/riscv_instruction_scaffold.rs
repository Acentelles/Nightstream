...
use deprecated_neo_memory::riscv::instruction::{
...
use deprecated_neo_vm_trace::{trace_program, Twist, TwistOpKind};
...
fn rv64_legacy_u64_shout_transport_rejects_full_width_direct_keys() {
    ...
    let _ = deprecated_neo_vm_trace::Shout::lookup(&mut shout, eq_id, key_u64);
}
...
fn rv64_virtual_word_helper_lookup_results_ignore_high64_signfill() {
    ...
    for op in [
        ...
        let key = encode_lookup_key_with_mode(op, lhs, rhs, 64, /*use_operand_mode_keys=*/ true);
        let out = deprecated_neo_vm_trace::Shout::lookup(&mut shout, shout_id, key);
        assert_eq!(
            out,
    ...
    let movsign_out = deprecated_neo_vm_trace::Shout::lookup(&mut shout, movsign_id, movsign_key);
...
fn mul_lookup_semantics_match_expected_words_in_rollout_mode() {
    ...
    let mul_out = deprecated_neo_vm_trace::Shout::lookup(&mut shout, mul_id, key);
    let mulhu_out = deprecated_neo_vm_trace::Shout::lookup(&mut shout, mulhu_id, key);
...
fn mul_decomposition_sequence_matches_shape() {
    ...
    let seq = deprecated_neo_memory::riscv::instruction::mul::decomposition_sequence(
...
fn mulhu_decomposition_sequence_matches_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::mulhu::decomposition_sequence(
...
fn mulw_decomposition_sequence_uses_helper_mask_then_local_compose() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::mulw::decomposition_sequence(
        /*rd=*/ 7, /*rs1=*/ 1, /*rs2=*/ 2, &mut alloc,
    );
    assert_eq!(seq.len(), 4);
    assert_eq!(
...
fn divuw_decomposition_sequence_uses_helper_mask_then_local_compose() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::divuw::decomposition_sequence(
        /*rd=*/ 7, /*rs1=*/ 1, /*rs2=*/ 2, &mut alloc,
    );
    assert_eq!(seq.len(), 4);
    assert_eq!(
...
fn divw_decomposition_sequence_uses_helper_mask_then_local_compose() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::divw::decomposition_sequence(
        /*rd=*/ 7, /*rs1=*/ 1, /*rs2=*/ 2, &mut alloc,
    );
    assert_eq!(seq.len(), 4);
    assert_eq!(
...
fn remuw_decomposition_sequence_uses_helper_mask_then_local_compose() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::remuw::decomposition_sequence(
        /*rd=*/ 7, /*rs1=*/ 1, /*rs2=*/ 2, &mut alloc,
    );
    assert_eq!(seq.len(), 4);
    assert_eq!(
...
fn remw_decomposition_sequence_uses_helper_mask_then_local_compose() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::remw::decomposition_sequence(
        /*rd=*/ 7, /*rs1=*/ 1, /*rs2=*/ 2, &mut alloc,
    );
    assert_eq!(seq.len(), 4);
    assert_eq!(
...
fn mulh_decomposition_sequence_matches_jolt_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::mulh::decomposition_sequence(
        /*rd=*/ 5, /*rs1=*/ 3, /*rs2=*/ 4, &mut alloc,
    );
    assert_eq!(seq.len(), 8);
...
fn mulhsu_decomposition_sequence_matches_jolt_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::mulhsu::decomposition_sequence(
        /*rd=*/ 7, /*rs1=*/ 1, /*rs2=*/ 2, &mut alloc,
    );
...
fn divu_decomposition_sequence_matches_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::divu::decomposition_sequence(
...
fn remu_decomposition_sequence_matches_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::remu::decomposition_sequence(
...
fn div_decomposition_sequence_matches_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::div::decomposition_sequence(
...
fn rem_decomposition_sequence_matches_shape() {
    let mut alloc = VirtualRegisterAllocator::new();
    let seq = deprecated_neo_memory::riscv::instruction::rem::decomposition_sequence(
...
fn mul_decomposition_sequence_matches_mul_semantics_on_samples() {
    ...
    for (lhs, rhs) in cases {
        ...
        let mut alloc = VirtualRegisterAllocator::new();
        let seq = deprecated_neo_memory::riscv::instruction::mul::decomposition_sequence(rd, rs1, rs2, &mut alloc);
...
fn mulhu_decomposition_sequence_matches_mulhu_semantics_on_samples() {
    ...
    for (lhs, rhs) in cases {
        ...
        let mut alloc = VirtualRegisterAllocator::new();
        let seq = deprecated_neo_memory::riscv::instruction::mulhu::decomposition_sequence(rd, rs1, rs2, &mut alloc);
        let _ = simulate_decomposition_sequence(&seq, &mut regs, /*xlen=*/ 32);
...
fn mulh_decomposition_sequence_matches_mulh_semantics_on_samples() {
    ...
    for (lhs, rhs) in cases {
        ...
        let mut alloc = VirtualRegisterAllocator::new();
        let seq = deprecated_neo_memory::riscv::instruction::mulh::decomposition_sequence(rd, rs1, rs2, &mut alloc);
        let _ = simulate_decomposition_sequence(&seq, &mut regs, /*xlen=*/ 32);
...
fn mulhsu_decomposition_sequence_matches_mulhsu_semantics_on_samples() {
    ...
    for (lhs, rhs) in cases {
        ...
        let mut alloc = VirtualRegisterAllocator::new();
        let seq = deprecated_neo_memory::riscv::instruction::mulhsu::decomposition_sequence(rd, rs1, rs2, &mut alloc);
        let _ = simulate_decomposition_sequence(&seq, &mut regs, /*xlen=*/ 32);
