...
use neo_ccs::gadgets::commitment_opening::{
    build_commitment_lincomb_public_input, build_commitment_lincomb_witness,
    build_opening_witness, commitment_lincomb_ccs, commitment_opening_from_rows_ccs,
};
...
fn public_equality_accepts_equal() {
    ...
    check_ccs_rowwise_zero(&ccs, &x, &w)
        .expect("equal vectors should pass public equality check");
}
...
fn public_equality_rejects_unequal() {
    ...
    assert!(
        result.is_err(),
        "unequal vectors should fail public equality check"
    );
}
...
fn commitment_opening_round_trip() {
    ...
    check_ccs_rowwise_zero(&ccs, &x, &w)
        .expect("commitment opening round-trip should pass");
}
...
fn commitment_lincomb_round_trip() {
    ...
    check_ccs_rowwise_zero(&ccs, &x, &w)
        .expect("commitment lincomb round-trip should pass");
}
...
fn lincomb_witness_correctness() {
    ...
    let x = build_commitment_lincomb_public_input(rho, &c_prev, &c_step, &c_next);
    check_ccs_rowwise_zero(&ccs, &x, &w)
        .expect("lincomb witness should satisfy the CCS");
}
