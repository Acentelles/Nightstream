#![allow(non_snake_case)]

use neo_fold::riscv_trace_shard::Rv32TraceWiring;
use neo_math::F;
use neo_memory::riscv::lookups::{encode_program, RiscvInstruction, RiscvOpcode};
use p3_field::PrimeCharacteristicRing;

#[test]
fn riscv_trace_wiring_ccs_shared_cpu_bus_shout_mulhu_prove_verify() {
    // Program:
    // - LUI x1, 16          (x1 = 65536)
    // - LUI x2, 16          (x2 = 65536)
    // - MULHU x3, x1, x2    (hi = 1)
    // - MULHU x4, x2, x1    (hi = 1)
    // - HALT
    let program = vec![
        RiscvInstruction::Lui { rd: 1, imm: 16 },
        RiscvInstruction::Lui { rd: 2, imm: 16 },
        RiscvInstruction::RAlu {
            op: RiscvOpcode::Mulhu,
            rd: 3,
            rs1: 1,
            rs2: 2,
        },
        RiscvInstruction::RAlu {
            op: RiscvOpcode::Mulhu,
            rd: 4,
            rs1: 2,
            rs2: 1,
        },
        RiscvInstruction::Halt,
    ];
    let program_bytes = encode_program(&program);

    let mut run = Rv32TraceWiring::from_rom(/*program_base=*/ 0, &program_bytes)
        .chunk_rows(1)
        .max_steps(program.len())
        .reg_output_claim(/*reg=*/ 3, F::from_u64(1))
        .reg_output_claim(/*reg=*/ 4, F::from_u64(1))
        .prove()
        .expect("rv32 trace prove (booleanity/trace-opening route, MULHU)");
    run.verify().expect("rv32 trace verify (booleanity/trace-opening route, MULHU)");
}
