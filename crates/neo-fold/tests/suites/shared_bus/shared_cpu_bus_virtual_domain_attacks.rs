...
fn tamper_reg_domain_bit_opening(run: &Rv32TraceWiringRun, proof: &mut ShardProof, lane_idx: usize, write_bit: bool) {
    ...
    let open_idx = step
        ...
        .openings
        .iter()
        .position(|opening| opening.point == r_time && opening.col_ids.iter().any(|&c| c == bit_col))
        .or_else(|| step.fold.openings.iter().position(|opening| opening.point == r_time))
