...
fn cpu_sumcheck_from_batched_time(
    batched_time: &crate::shard_proof_types::BatchedTimeProof,
    r_time: &[K],
) -> Result<crate::shard_proof_types::CpuTimeSumcheckProof, PiCcsError> {
    let label = batched_time
        .labels
        .first()
        .copied()
        .ok_or_else(|| PiCcsError::ProtocolError("missing batched-time CCS/time label".into()))?;
    if label != b"ccs/time" {
        return Err(PiCcsError::ProtocolError(format!(
            "expected batched-time claim 0 to be ccs/time, got {:?}",
            label
        )));
    }
    let claimed_sum = *batched_time
        .claimed_sums
        .first()
        .ok_or_else(|| PiCcsError::ProtocolError("missing batched-time CCS/time claimed_sum".into()))?;
    let round_polys = batched_time
        .round_polys
        .first()
        .cloned()
        .ok_or_else(|| PiCcsError::ProtocolError("missing batched-time CCS/time rounds".into()))?;
    Ok(crate::shard_proof_types::CpuTimeSumcheckProof {
        claimed_sum,
        round_polys,
    ...
    for (idx, step) in steps.iter().enumerate() {
                    ...
                    if let Some(child_cs) = shared_val_lane_child_cs.as_ref() {
                        ...
                        if cpu_bus.bus_cols > 0 {
                            ...
                            let parent_has_prefilled_bus =
                                rlc_parent.y.len() > core_t || rlc_parent.y_scalars.len() > core_t;
                            if wit.cols() == cpu_bus.m && !parent_has_prefilled_bus {
                                crate::memory_sidecar::cpu_bus::append_bus_openings_to_me_instance(
                                    params,
                                    &cpu_bus,
                                    core_t,
                                ...
                                if rlc_parent.y.len() == core_t && rlc_parent.y_scalars.len() == core_t {
                                    crate::memory_sidecar::cpu_bus::append_zero_bus_openings_to_me_instance(
                                        params,
                                        &cpu_bus,
                                        core_t,
                                ...
                                } else if rlc_parent.y.len() != want_len || rlc_parent.y_scalars.len() != want_len {
                                    return Err(PiCcsError::ProtocolError(format!(
                                        "step {}: val-lane non-physical bus path expects exact parent y/y_scalars len {} (got y.len()={}, y_scalars.len()={})",
                                        step_idx,
                                        want_len,
                                        rlc_parent.y.len(),
                                ...
                                for (child_idx, child) in dec_children.iter_mut().enumerate() {
                                    if child.y.len() < core_t || child.y_scalars.len() < core_t {
                                        return Err(PiCcsError::ProtocolError(format!(
                                            "step {}: val-lane non-physical bus path expects child y/y_scalars len >= core_t={} (got y.len()={}, y_scalars.len()={})",
                                            step_idx,
                                            core_t,
                                            child.y.len(),
                                            child.y_scalars.len()
                                    ...
                                    child.y.truncate(core_t);
                                    child.y_scalars.truncate(core_t);
                                    ...
                                    for col_id in 0..cpu_bus.bus_cols {
                                        if child_idx == 0 {
                                            child.y.push(rlc_parent.y[core_t + col_id].clone());
                                            child.y_scalars.push(rlc_parent.y_scalars[core_t + col_id]);
                                        } else {
                                            child.y.push(vec![K::ZERO; y_pad]);
                                            child.y_scalars.push(K::ZERO);
                                        }
                                    }
                                    if child_idx > 0 {
                                        debug_assert!(
                                            child.y[core_t..]
                                                ...
                                                && child.y_scalars[core_t..].iter().all(|v| *v == K::ZERO),
                                            "non-primary val-lane DEC children must keep propagated metadata openings at zero"
                                        );
                                    }
                                    if child.y.len() != want_len || child.y_scalars.len() != want_len {
                                        return Err(PiCcsError::ProtocolError(format!(
                                            "step {}: val-lane child suffix-length drift (child y/y_scalars={}/{}, expected={})",
                                            step_idx,
                                            child.y.len(),
                                            child.y_scalars.len(),
                                            want_len
        ...
        if !mem_proof.wb_me_claims.is_empty() {
            ...
            for (claim_idx, me) in mem_proof.wb_me_claims.iter().enumerate() {
                ...
                if rlc_parent.y.len() != want_len || rlc_parent.y_scalars.len() != want_len {
                    return Err(PiCcsError::ProtocolError(format!(
                        "step {}: WB fold expects exact parent y/y_scalars len {} (got y.len()={}, y_scalars.len()={})",
                        step_idx,
                        want_len,
                        rlc_parent.y.len(),
                        rlc_parent.y_scalars.len()
                ...
                for (child_idx, child) in dec_children.iter_mut().enumerate() {
                    if child.y.len() < core_t || child.y_scalars.len() < core_t {
                        return Err(PiCcsError::ProtocolError(format!(
                            "step {}: WB fold expects child y/y_scalars len >= core_t={} (got y.len()={}, y_scalars.len()={})",
                            step_idx,
                            core_t,
                            child.y.len(),
                            child.y_scalars.len()
                    ...
                    for open_idx in 0..wb_cols.len() {
                        if child_idx == 0 {
                            child.y.push(rlc_parent.y[core_t + open_idx].clone());
                            child
                                .y_scalars
                                .push(rlc_parent.y_scalars[core_t + open_idx]);
                        } else {
                            child.y.push(vec![K::ZERO; y_pad]);
                            child.y_scalars.push(K::ZERO);
                        }
                    ...
                    if child.y.len() != want_len || child.y_scalars.len() != want_len {
                        return Err(PiCcsError::ProtocolError(format!(
                            "step {}: WB fold child suffix-length drift (child y/y_scalars={}/{}, expected={})",
                            step_idx,
                            child.y.len(),
                            child.y_scalars.len(),
                            want_len
        ...
        if !mem_proof.wp_me_claims.is_empty() {
            ...
            for (claim_idx, me) in mem_proof.wp_me_claims.iter().enumerate() {
                ...
                if rlc_parent.y.len() != want_len || rlc_parent.y_scalars.len() != want_len {
                    return Err(PiCcsError::ProtocolError(format!(
                        "step {}: WP fold expects exact parent y/y_scalars len {} (got y.len()={}, y_scalars.len()={})",
                        step_idx,
                        want_len,
                        rlc_parent.y.len(),
                        rlc_parent.y_scalars.len()
                ...
                for (child_idx, child) in dec_children.iter_mut().enumerate() {
                    if child.y.len() < core_t || child.y_scalars.len() < core_t {
                        return Err(PiCcsError::ProtocolError(format!(
                            "step {}: WP fold expects child y/y_scalars len >= core_t={} (got y.len()={}, y_scalars.len()={})",
                            step_idx,
                            core_t,
                            child.y.len(),
                            child.y_scalars.len()
                    ...
                    if child.y.len() != want_len || child.y_scalars.len() != want_len {
                        return Err(PiCcsError::ProtocolError(format!(
                            "step {}: WP fold child suffix-length drift (child y/y_scalars={}/{}, expected={})",
                            step_idx,
                            child.y.len(),
                            child.y_scalars.len(),
                            want_len
        ...
        let opening_proofs = {
            ...
            for opening in fold_openings.iter() {
                ...
                for (col_id, eval) in pairs.into_iter() {
                    ...
                    let digits = crate::time_opening::me_adapter::eval_time_mat_digits_at_point(
                        domain,
        ...
        let cpu_sumcheck = cpu_sumcheck_from_batched_time(&batched_time, &r_time)?;
        ...
        step_proofs.push(StepProof {
            fold: FoldStep {
                ...
                memory_time_proofs: batched_time.labels.iter().skip(1).copied().collect(),
