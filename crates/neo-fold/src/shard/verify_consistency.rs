...
pub(crate) fn validate_step_time_openings_consistency(
    ...
    let requires_r_time_named_openings =
        cpu_bus.bus_cols > 0 || !step.mem_insts.is_empty() || !step.lut_insts.is_empty();
    if requires_r_time_named_openings {
        let ccs0 = step_proof
            .fold
            .ccs_out
            .first()
            .ok_or_else(|| PiCcsError::ProtocolError("verify/openings: missing ccs_out[0]".into()))?;
        if ccs0.r.as_slice() != r_time {
            return Err(PiCcsError::ProtocolError(
                "verify/openings: ccs_out[0].r does not match derived r_time".into(),
            ));
        }
    }
...
pub(crate) fn validate_time_sumcheck_metadata(
    ...
    if labels.first().copied() != Some(b"ccs/time") {
        return Err(PiCcsError::ProtocolError(format!(
            "step {}: expected batched_time claim 0 label ccs/time",
            step_idx
        )));
    }
    let expected_cpu_sum = *claimed_sums
        .first()
        .ok_or_else(|| PiCcsError::ProtocolError(format!("step {}: missing batched_time claim 0 sum", step_idx)))?;
    let expected_cpu_rounds = round_polys
        .first()
        .ok_or_else(|| PiCcsError::ProtocolError(format!("step {}: missing batched_time claim 0 rounds", step_idx)))?;
    ...
    if step_proof.fold.cpu_sumcheck.round_polys != *expected_cpu_rounds {
        return Err(PiCcsError::ProtocolError(format!(
            "step {}: cpu_sumcheck round_polys mismatch",
            step_idx
        )));
    }
    if step_proof.fold.cpu_sumcheck.r_time.as_slice() != r_time {
        return Err(PiCcsError::ProtocolError(format!(
    ...
    if step_proof.fold.shift_sumcheck.r_time.as_slice() != r_time {
        return Err(PiCcsError::ProtocolError(format!(
