    ...
    MB: Fn(&[Cmt], u32) -> Cmt + Clone + Copy,
{
    ...
    if let Some(bus) = cpu_bus {
        if bus.bus_cols > 0 {
            let core_t = s.t();
            let parent_has_prefilled_bus = rlc_parent.y.len() > core_t || rlc_parent.y_scalars.len() > core_t;
            if Z_mix.cols() == bus.m && !parent_has_prefilled_bus {
                ...
                if rlc_parent.y.len() < core_t || rlc_parent.y_scalars.len() < core_t {
                    return Err(PiCcsError::ProtocolError(format!(
                        "step {}: non-physical bus path expects parent y/y_scalars len to be at least core_t={} (got y.len()={}, y_scalars.len()={})",
                        step_idx,
                        core_t,
                        rlc_parent.y.len(),
                ...
                let parent_extra = rlc_parent.y.len().saturating_sub(core_t);
                ...
                if !matches!(lane, RlcLane::Main) || trace_linkage_t_len.is_none() {
                    if parent_extra != bus.bus_cols || rlc_parent.y_scalars.len() != core_t + bus.bus_cols {
                        return Err(PiCcsError::ProtocolError(format!(
                            "step {}: non-physical bus path requires exact parent suffix length (y extra={}, y_scalars extra={}, expected bus_cols={})",
                            step_idx,
                            parent_extra,
                            rlc_parent.y_scalars.len().saturating_sub(core_t),
                            bus.bus_cols
                        )));
                    }
                }
                ...
                for (child_idx, child) in dec_children.iter_mut().enumerate() {
                    if child.y.len() < core_t || child.y_scalars.len() < core_t {
                        return Err(PiCcsError::ProtocolError(format!(
                            "step {}: non-physical bus path expects child y/y_scalars len to start at core_t={} (got y.len()={}, y_scalars.len()={})",
                            step_idx,
                            core_t,
                            child.y.len(),
                            child.y_scalars.len()
    ...
    if matches!(lane, RlcLane::Main) && trace_linkage_t_len.is_some() {
        ...
        if rlc_parent.y.len() >= trace_open_base && rlc_parent.y_scalars.len() >= trace_open_base {
            ...
            let trace_available = rlc_parent.y.len().saturating_sub(trace_open_base);
            if trace_available != trace_cols_to_open.len()
                || rlc_parent.y_scalars.len().saturating_sub(trace_open_base) != trace_cols_to_open.len()
            {
                return Err(PiCcsError::ProtocolError(format!(
                    "step {}: trace linkage propagation requires exact trace suffix length (y extra={}, y_scalars extra={}, expected={})",
                    step_idx,
                    trace_available,
                    rlc_parent.y_scalars.len().saturating_sub(trace_open_base),
                    trace_cols_to_open.len()
                )));
            }
            let y_pad = (params.d as usize).next_power_of_two();
            for (child_idx, child) in dec_children.iter_mut().enumerate() {
                if child.y.len() < trace_open_base || child.y_scalars.len() < trace_open_base {
                    return Err(PiCcsError::ProtocolError(format!(
                        "step {}: trace linkage propagation expects child y/y_scalars len to start at trace_open_base={} (got y.len()={}, y_scalars.len()={})",
                        step_idx,
                        trace_open_base,
                        child.y.len(),
                        child.y_scalars.len()
                ...
                child.y.truncate(trace_open_base);
                child.y_scalars.truncate(trace_open_base);
                for open_idx in 0..trace_available {
                    if child_idx == 0 {
                        child
                            .y
                            .push(rlc_parent.y[trace_open_base + open_idx].clone());
                        child
                            .y_scalars
                            .push(rlc_parent.y_scalars[trace_open_base + open_idx]);
                    } else {
                ...
                if child_idx > 0 {
                    debug_assert!(
                        child.y[trace_open_base..]
                            .iter()
                            .all(|row| row.iter().all(|v| *v == K::ZERO))
                            && child.y_scalars[trace_open_base..]
                                .iter()
                                .all(|v| *v == K::ZERO),
    ...
    for (child_idx, child) in dec_children.iter().enumerate() {
        if child.y.len() != rlc_parent.y.len() || child.y_scalars.len() != rlc_parent.y_scalars.len() {
            return Err(PiCcsError::ProtocolError(format!(
                "step {}: DEC child[{}] suffix-length drift after propagation (child y/y_scalars={}/{}, parent y/y_scalars={}/{})",
                step_idx,
                child_idx,
                child.y.len(),
                child.y_scalars.len(),
    ...
    L: SModuleHomomorphism<F, Cmt> + Sync,
{
    ...
    if cfg.outputs {
        ...
        for (idx, (a, b)) in out_me_ref.iter().zip(ccs_out.iter()).enumerate() {
            ...
            // different per-digit y-vectors while preserving canonical y_scalars.
