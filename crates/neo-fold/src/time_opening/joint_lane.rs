...
use crate::time_opening::me_adapter::{
    ...
    recompose_digits_to_scalar,
};
use crate::time_opening::reduction::bind_opening_reduction_and_sample_group_coeffs;
use crate::time_opening::stage8_cache::{
    build_claim_witness_from_cache, collect_opening_proof_col_ids, precompute_stage8_encoded_columns_for_col_ids,
    validate_encoded_stage8_column_domain, EncodedStage8ColumnCache,
};
...
#[inline]
fn allow_stage8_commit_acceleration_for_group_count(
    backend_ctx: &neo_reductions::accelerator::BackendContext,
    group_count: usize,
) -> bool {
    match backend_ctx.selected_device_api() {
        // Note-spend's Stage-8 group path is still too fragmented on Metal to justify
        // offload. Keep small group batches on CPU until we have a truly resident kernel.
        Some(neo_gpu::DeviceApi::Metal) => group_count >= 8,
        // The papers' sparse-dense prover lesson applies here too: tiny Stage-8 batches do not
        // amortize CUDA launch/mix overhead, even in strict backend mode.
        Some(neo_gpu::DeviceApi::Cuda) if group_count < 4 => false,
        Some(neo_gpu::DeviceApi::Cuda) => {
            backend_ctx.mojo_required()
                || !matches!(
                    backend_ctx.commit_many_execution_status(group_count.saturating_mul(D)),
...
#[inline]
pub(crate) fn left_mul_add_row_major_into(
    dst: &mut Mat<F>,
    rho: &Mat<F>,
    src_data: &[F],
    m: usize,
...
fn build_stage8_group_clusters(groups: &[JointOpeningGroupProof]) -> Vec<Vec<usize>> {
    if can_use_unified_stage8_witness(groups) {
        return vec![(0..groups.len()).collect()];
    }
...
fn fold_stage8_commitments_with_mixers(rhos: &[Mat<F>], commits: &[Cmt], context: &str) -> Result<Cmt, PiCcsError> {
    let mut acc: Option<Cmt> = None;
    for (rho, commit) in rhos.iter().zip(commits.iter()) {
        add_rot_scaled_commitment(&mut acc, commit, rho)?;
    }
    acc.ok_or_else(|| PiCcsError::ProtocolError(format!("{context}: missing fallback commitment")))
}
...
fn mix_group_witnesses(
    ...
    let wit_refs = group_wits.iter().collect::<Vec<_>>();
    if let Some(accel_out) =
        accel::try_mix_witness_refs_with_backend(backend_ctx, wit_refs.as_slice(), mix_rhos, time_t)?
    {
        return Ok(accel_out);
    }
...
pub fn prove_joint_opening_lane_with_witnesses_and_metrics(
    ...
    let prepare_start = time_now();
    let logical_col_pos = build_logical_col_pos(time_col_ids)?;
    let encoded_stage8_cols = precompute_stage8_encoded_columns_for_col_ids(
        params,
        step,
        &logical_col_pos,
        time_cpu_commitments.len(),
        collect_opening_proof_col_ids(opening_proofs),
    )?;
    let mut out = prove_joint_opening_lane_with_precomputed_columns_and_metrics(
        tr,
        params,
        step_idx,
        step,
    ...
    out.2.prepare += elapsed_duration(prepare_start);
    Ok(out)
}

pub(crate) fn prove_joint_opening_lane_with_precomputed_columns_and_metrics(
    tr: &mut Poseidon2Transcript,
    params: &NeoParams,
    step_idx: usize,
    step: &StepWitnessBundle<Cmt, F, K>,
    ...
    opening_unification: &OpeningUnificationProof,
    claim_eta_coeffs: &[Vec<Mat<F>>],
    claim_eta_flat_rq_coeffs: Option<&[Vec<neo_gpu::FlatRq>]>,
    ...
    metrics.prepare += elapsed_duration(prepare_start);
    ...
    let allow_commit_accel = allow_stage8_commit_acceleration_for_group_count(backend_ctx, group_results.len());
    let expected_commitments_start = time_now();
    let expected_commitments = if allow_commit_accel {
        crate::shard::mix_many_rhos_commits_with_backend_result(
            backend_ctx,
            |mix_rhos, commits| fold_stage8_commitments_with_mixers(mix_rhos, commits, "stage8 batched group fallback"),
            &group_results
                .iter()
    ...
    metrics.expected_commitments += elapsed_duration(expected_commitments_start);
    ...
    let joint_commitments = expected_commitments.clone();
        ...
        .enumerate()
    {
        ...
        out_groups.push(JointOpeningGroupProof {
            ...
            update_class_digest: group_work.update_class_digest,
    ...
    if !out_groups.is_empty() {
        ...
        if !can_unify {
            ...
            for (cluster_idx, group_indices) in cluster_layouts.iter().enumerate() {
                ...
                let homogeneous_point_domain = member_groups
                    .iter()
                    .all(|group| group.point == member_groups[0].point && group.domain == member_groups[0].domain);
                ...
                let expected_commitment = if allow_commit_accel {
                    crate::shard::mix_rhos_commits_with_backend_result(
                        backend_ctx,
                        |rhos, commits| {
                            fold_stage8_commitments_with_mixers(rhos, commits, "stage8 cluster fold fallback")
                        },
                        mix_rhos.as_slice(),
                        &member_groups
                            .iter()
                ...
                let cluster_group_wits: Vec<Mat<F>> = group_indices
                    ...
                    .collect();
                let cluster_wit = mix_group_witnesses(backend_ctx, &cluster_group_wits, &mix_rhos, t)?;
                ...
                let (cluster_point, cluster_domain, cluster_claim_digits) = if homogeneous_point_domain {
                    let cluster_anchor = member_groups[0];
                    (
                        cluster_anchor.point.clone(),
                        cluster_anchor.domain,
                        expected_claim_digits.clone(),
                    )
                } else {
                    (
        ...
        let expected_commitment = if allow_commit_accel {
            crate::shard::mix_rhos_commits_with_backend_result(
                backend_ctx,
                |rhos, commits| fold_stage8_commitments_with_mixers(rhos, commits, "stage8 unified fold fallback"),
                mix_rhos.as_slice(),
                &out_groups
                    .iter()
        ...
        let (unified_point, unified_domain, unified_commitment, unified_claim_digits) = if can_unify {
            let unified_z = mix_group_witnesses(backend_ctx, &out_wits, &mix_rhos, t)?;
        ...
        unified_fold = Some(JointOpeningGroupProof {
            ...
            update_class_digest: crate::time_opening::reduction::combined_update_class_digest(
                &out_groups
                    .iter()
                    .map(|group| group.update_class_digest)
                    .collect::<Vec<_>>(),
            ),
        ...
        if can_unify {
            let unified_fold_ref = unified_fold.as_ref().ok_or_else(|| {
                PiCcsError::ProtocolError("stage8 cluster unified fold missing for collapsible groups".into())
            })?;
            stage8_clusters.push(Stage8ClusterProof {
                ...
                joint_claim_digits: unified_fold_ref.joint_claim_digits.clone(),
                joint_claim: unified_fold_ref.joint_claim,
                joint_commitment: unified_fold_ref.joint_commitment.clone(),
            });
        }
...
pub fn verify_joint_opening_lane(
    ...
    for (group_idx, (group, pf_group)) in reduction.groups.iter().zip(lane.groups.iter()).enumerate() {
        ...
        let expected_update_class_digest =
            crate::time_opening::reduction::update_class_digest(group.claim_indices.as_slice(), claim_eta_coeffs)?;
        if pf_group.update_class_digest != expected_update_class_digest {
            return Err(PiCcsError::ProtocolError(format!(
                "time/opening joint/verify: group {} update_class_digest mismatch",
                group_idx
            )));
        }
    }
        ...
        .enumerate()
    {
        ...
        let homogeneous_point_domain = member_groups
            .iter()
            .all(|group| group.point == anchor.point && group.domain == anchor.domain);
        ...
        } else if cluster.point.as_slice() != opening_unification.r_unify.as_slice()
            || cluster.domain != OpeningDomain::Cpu
        {
            return Err(PiCcsError::ProtocolError(format!(
                "time/opening joint/verify: stage8 cluster {} must use synthetic (r_unify, Cpu) anchor",
                cluster_idx
            )));
        }
