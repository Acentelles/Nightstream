...
use neo_ccs::{CcsMatrix, CcsStructure, CscMat, Mat, MeInstance, SModuleHomomorphism, SparsePoly, Term};
...
pub struct Stage8FoldLanePlan {
    ...
    pub claims: Vec<MeInstance<Cmt, F, K>>,
}
...
fn mix_group_witnesses(group_wits: &[Mat<F>], mix_rhos: &[Mat<F>], time_t: usize) -> Result<Mat<F>, PiCcsError> {
    ...
    for (rho, wit) in mix_rhos.iter().zip(group_wits.iter()) {
        if wit.rows() != D || wit.cols() != time_t {
            return Err(PiCcsError::ProtocolError(format!(
                "stage8 unified fold: witness shape mismatch (got {}x{}, expected {}x{})",
                wit.rows(),
                wit.cols(),
                D,
                time_t
        ...
        left_mul_add_into(&mut out, rho, wit)?;
    }
    Ok(out)
}
...
pub fn prove_joint_opening_lane_with_witnesses(
    ...
    let mut out_groups = Vec::with_capacity(reduction.groups.len());
    let mut out_wits = Vec::with_capacity(reduction.groups.len());
