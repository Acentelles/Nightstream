...
fn w2_stage_gate(remaining: K, target: u64, max_remaining: u64) -> K {
    let mut gate = K::ONE;
    for r in 1..=max_remaining {
        if r != target {
            gate *= remaining - K::from(F::from_u64(r));
        }
    }
    gate
}
...
#[inline]
fn w2_remaining_domain_poly(remaining: K, max_remaining: u64) -> K {
    let mut poly = K::ONE;
    for r in 1..=max_remaining {
        poly *= remaining - K::from(F::from_u64(r));
    }
    poly
}
...
pub(crate) fn w2_alu_branch_lookup_residuals(
    ...
    let mut residuals = Vec::with_capacity(700);
    residuals.extend(raw.map(|r| non_virtual * r));
    ...
    residuals.push(is_virtual * op_mulh * w2_remaining_domain_poly(rem, 7));
    residuals.push(is_virtual * op_mulhsu * w2_remaining_domain_poly(rem, 11));
    ...
    for row in mulh_rows {
        let gate = is_virtual * op_mulh * w2_stage_gate(rem, row.remaining, 7);
    ...
    for row in mulhsu_rows {
        let gate = is_virtual * op_mulhsu * w2_stage_gate(rem, row.remaining, 11);
        push_virtual_stage_row(&mut residuals, gate, row);
    }
    ...
    residuals.push(is_virtual * op_div * w2_remaining_domain_poly(rem, 18));
    residuals.push(is_virtual * op_divu * w2_remaining_domain_poly(rem, 8));
    residuals.push(is_virtual * op_rem * w2_remaining_domain_poly(rem, 19));
    residuals.push(is_virtual * op_remu * w2_remaining_domain_poly(rem, 7));
    ...
    for row in div_rows {
        let gate = is_virtual * op_div * w2_stage_gate(rem, row.remaining, 18);
    ...
    for row in rem_rows {
        let gate = is_virtual * op_rem * w2_stage_gate(rem, row.remaining, 19);
        push_virtual_stage_sparse_row(&mut residuals, gate, row);
    }
    ...
    for row in divu_rows {
        let gate = is_virtual * op_divu * w2_stage_gate(rem, row.remaining, 8);
        push_virtual_stage_row(&mut residuals, gate, row);
    }
    ...
    for row in remu_rows {
        let gate = is_virtual * op_remu * w2_stage_gate(rem, row.remaining, 7);
        push_virtual_stage_row(&mut residuals, gate, row);
    }
