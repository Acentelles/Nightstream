...
pub fn verify_route_a_memory_step(
    ...
    let is_virtual_open = if trace_mode {
        ...
        let is_virtual_idx = wp_cols
            .iter()
            .position(|&c| c == trace_layout.is_virtual)
            .ok_or_else(|| PiCcsError::ProtocolError("virtual-domain check: missing is_virtual WP column".into()))?;
        let is_virtual_open_idx = core_t.checked_add(is_virtual_idx).ok_or_else(|| {
            PiCcsError::InvalidInput("virtual-domain check: is_virtual opening index overflow".into())
        })?;
        wp_me
            .y_scalars
            .get(is_virtual_open_idx)
            .copied()
            .ok_or_else(|| PiCcsError::ProtocolError("virtual-domain check: missing is_virtual WP opening".into()))?
    } else {
        K::ZERO
    };
    ...
    if !step.lut_insts.is_empty() && enforce_trace_shout_linkage {
        ...
        let expected_table_id = expected_trace_shout_table_id_from_openings(core_t, step, mem_proof, r_time)?;
