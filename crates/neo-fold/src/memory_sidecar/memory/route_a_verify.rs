...
pub fn verify_route_a_memory_step(
    ...
    let (bus_val_open_cur, bus_val_open_prev, has_prev_val_claim) = if step.mem_insts.is_empty() {
        ...
        let bus_open_prev = if let Some(_cpu_me_prev) = cpu_me_prev {
            ...
            let prev_openings = prev_step_time_openings.ok_or_else(|| {
                PiCcsError::ProtocolError(
                    "route-a/val prev: missing prev_step_time_openings in canonical committed mode".into(),
                )
            })?;
            ...
            let prev_entry = require_time_opening_entry_for_point(
                prev_openings,
                r_val.as_slice(),
            ...
            let logical_prev = require_time_openings_for_point(
                prev_openings,
                r_val.as_slice(),
                prev_bus_col_ids.as_slice(),
                "route-a/val prev",
