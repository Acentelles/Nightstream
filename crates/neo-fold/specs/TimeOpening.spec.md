...
- **What it is**: The stage-8 and time-opening layer that turns time-column obligations into opening manifests, grouped reductions, joint-lane proofs, and final opening unification checks.
...
- **Layer**: extension
- **Direct paper theorem owner?** Yes, for the Twist/Shout-derived time-opening and stage-8 extension semantics. It is not itself a Section 7 theorem owner.
...
| stage-8 fold-lane opening proof | implementation support for the SuperNeo route | `Stage8FoldLanePlan`, `build_stage8_fold_lane_plan`, `prove_joint_opening_lane*`, `verify_joint_opening_lane` | One owner for the stage-8 joint opening lane |
...
| decomposition base for stage-8 time openings | implementation support | `STAGE8_TIME_DECOMP_BASE` | Shared decomposition base used by the stage-8 path |
...
- `docs/architecture/how-superneo-works.md`
  - for how stage-8/opening obligations fit the overall Rust proving pipeline
...
|---|---|---|---|
| `STAGE8_TIME_DECOMP_BASE` | const | Core | Shared decomposition base for stage-8 time-opening logic; prover and verifier must agree on it exactly |
...
| Rust symbol | Kind | Role | Contract |
|---|---|---|---|
| `Stage8FoldLanePlan` | struct | Core | The explicit stage-8 plan for one joint opening lane |
| `build_stage8_fold_lane_plan` | fn | Core | Canonical planner for stage-8 joint-lane obligations |
...
| `TO-2` | Reduction groups and sampled coefficients preserve the same opening obligations, not weaker surrogates | Artifact validation over exported opening artifacts | Prevents stage-8 weakening |
| `TO-3` | Joint-lane planning preserves lane/domain separation and required openings | `NeoFoldRelationValidation` item `3`; `paperArtifactRelationChecks`; `paperArtifactRelationChecks_implies_paperArtifactStepRelationsAccepts` | Prevents lane-crossing bugs in the projected stage-8 relation |
...
| `TO-6` | `STAGE8_TIME_DECOMP_BASE` remains the agreed decomposition base | Rust constant tests + artifact validation | Prevents prover/verifier mismatch on the stage-8 path |
...
|---|---|
| exported valid stage-8/opening artifacts | accepted by Lean artifact validators |
| tampered stage-8/opening artifacts | rejected by Lean artifact validators |
| projected paper-core relation checks over valid artifacts | remain true after erasing Rust-only sidecars |

## Quality Expectations

- Opening manifests and grouped reductions must remain the single source of truth for stage-8 claim structure.
...
3. Exported stage-8/opening artifacts are accepted by the Rust↔Lean validators.
4. The stage-8 path preserves the main-lane/val-lane split and does not weaken the final opening obligations.
