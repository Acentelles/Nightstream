    ...
    Comb: Fn(&[Mat<F>], &[Cmt]) -> Cmt,
{
    ...
    //
    // Keep this shortcut only for non-paper modes. PaperExact must remain a direct formula mirror.
    let allow_k1_fast_path = {
        #[cfg(feature = "paper-exact")]
        {
            !matches!(mode, FoldingMode::PaperExact)
        }
    ...
    if me_inputs.len() == 1 && allow_k1_fast_path {
        let inp = &me_inputs[0];
        ...
        let c = mix_commits(&rho_mats, &inputs_c);

        let ct = crate::common::ct_from_y_ring_for_ccs_m(&inp.y_ring, params, s.m);
        if !(inp.s_col.is_empty() && inp.y_zcol.is_empty()) {
            if inp.s_col.is_empty() || inp.y_zcol.is_empty() {
                return Err(PiCcsError::InvalidInput(
                    "rlc_with_commit(k=1): incomplete NC channel, expected both s_col and y_zcol".into(),
                ));
            }
            let d_pad = 1usize
                .checked_shl(ell_d as u32)
                .ok_or_else(|| PiCcsError::InvalidInput("rlc_with_commit(k=1): 2^ell_d overflow".into()))?;
            if inp.y_zcol.len() != d_pad {
                return Err(PiCcsError::InvalidInput(format!(
                    "rlc_with_commit(k=1): y_zcol.len()={} expected {}",
                    inp.y_zcol.len(),
                    d_pad
                )));
            }
    ...
    MR: Fn(&[Mat<F>], &[Cmt]) -> Cmt,
{
    ...
    if inputs.len() == 1 {
        ...
        let inputs_c = vec![inp.c.clone()];
        let c = mix_rhos_commits(&rho_mats, &inputs_c);
