...
use neo_midnight_bridge::relations::{
    PiCcsFeChunkAggSumcheckInstance, PiCcsFeChunkAggSumcheckRelation, PiCcsFeChunkAggSumcheckWitness, PiCcsFeChunkRelation,
    SparsePolyRepr, SparsePolyTermRepr,
};
...
fn mojo_plonk_pi_ccs_fe_sumcheck_agg_poseidon2_batch_1_from_snapshot_verifies_in_rust() -> anyhow::Result<()> {
    ...
    // CCS sparse polynomial f (for F' evaluation).
    ...
    // Choose the largest chunk size (in flattened (out_idx,j) pairs) that fits Midnight's cap.
    ...
    let mut chunk_sums: Vec<KRepr> = Vec::new();
    ...
    let rel = PiCcsFeChunkAggSumcheckRelation {
        ...
        poly: poly.clone(),
        start_idx: 0,
        count: 0, // sumcheck + terminal aggregate only (no chunk binding)
        n_chunks: chunk_sums.len(),
        chunk_index: 0,
    };
    ...
    let pi = PiCcsFeChunkAggSumcheckRelation::format_instance(&instance).expect("format_instance");
    let com_inst = PiCcsFeChunkAggSumcheckRelation::format_committed_instances(&witness);
    ...
    let pkg_path = dir.join("pi_ccs_fe_sumcheck_agg.nmbp");
    let ws_path = dir.join("pi_ccs_fe_sumcheck_agg.nmbws");
    let proof_path = dir.join("pi_ccs_fe_sumcheck_agg.proof.bin");
