...
use neo_midnight_bridge::relations::{PiCcsNcChunkAggSumcheckInstance, PiCcsNcChunkAggSumcheckRelation, PiCcsNcChunkAggSumcheckWitness, PiCcsNcChunkRelation};
...
fn mojo_plonk_pi_ccs_nc_chunk_agg_sumcheck_poseidon2_batch_1_from_snapshot_verifies_in_rust() -> anyhow::Result<()> {
    ...
    for start_i in (0..k_total).step_by(chunk_size) {
        ...
        chunk_sums.push(k_to_repr(&chunk_sum));
    }

    let agg_chunk_index = chunk_sums.len().saturating_sub(1);
    let agg_start_i = agg_chunk_index * chunk_size;
    let agg_count = cmp::min(chunk_size, k_total - agg_start_i);
    let agg_start_exp = agg_start_i + 1;
    let yz_slice = &y_zcols_all[agg_start_i..agg_start_i + agg_count];
    ...
    let rel = PiCcsNcChunkAggSumcheckRelation {
        ...
        ell_m,
        b: params_b,
        start_exp: agg_start_exp,
        count: agg_count,
        n_chunks: chunk_sums.len(),
        chunk_index: agg_chunk_index,
    };
    ...
    let pkg_path = dir.join("pi_ccs_nc_chunk_agg_sumcheck.nmbp");
    let ws_path = dir.join("pi_ccs_nc_chunk_agg_sumcheck.nmbws");
    let proof_path = dir.join("pi_ccs_nc_chunk_agg_sumcheck.proof.bin");
