...
#[test]
fn mojo_plonk_pi_ccs_sumcheck_nc_proof_from_snapshot_verifies_in_rust() -> anyhow::Result<()> {
    ...
    let initial_sum = k(11, 13);
    let mut claim = initial_sum;
    ...
    let pkg = export_package_v3(
        RelationKind::PiCcsSumcheckNc,
        &RelationParamsV1::PiCcsSumcheckNc {
            version: 1,
            n_rounds,
            poly_len,
        },
    ...
    let pi = PiCcsSumcheckNcRelation::format_instance(&instance).expect("format_instance");
    let com_inst = PiCcsSumcheckNcRelation::format_committed_instances(&witness);
    ...
    let pkg_path = dir.join("pi_ccs_sumcheck_nc.nmbp");
    let ws_path = dir.join("pi_ccs_sumcheck_nc.nmbws");
    let proof_path = dir.join("pi_ccs_sumcheck_nc.proof.bin");
