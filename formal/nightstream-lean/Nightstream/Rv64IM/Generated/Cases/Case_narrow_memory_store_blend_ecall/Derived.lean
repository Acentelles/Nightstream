...
def derivedCase : ParityDerivedCase :=
  ...
  , registerWrites := []
  , ramEvents := [{ traceIndex := 0, stepIndex := 0, kind := .write, addr := 16384, previous := 9833440827789222417, next := 9833440827789278993 }, { traceIndex := 1, stepIndex := 1, kind := .write, addr := 16384, previous := 9833440827789278993, next := 9833440826664156945 }, { traceIndex := 2, stepIndex := 2, kind := .write, addr := 16384, previous := 9833440826664156945, next := 1311761794802646801 }]
  ...
  , message := (bytes [69, 185, 103, 181, 219, 7, 84, 178, 33, 50, 181, 108, 252, 31, 114, 167, 165, 28, 113, 61, 140, 213, 232, 206, 71, 7, 48, 142, 115, 98, 75, 248])
  , u64s := []
  ...
  , cursorAfter := { stateWords := [39474052760708978, 40022254520559829, 4165689971, 13735928815302471318, 15174030263403373357, 2819236080141036144, 3879562584697143093, 5963323009994171033], absorbed := 3 }
  , challengeOutput := none
  , digestOutput := none
}, {
  kind := .challengeField
  ...
  , message := (bytes [])
  , u64s := []
  , cursorBefore := { stateWords := [39474052760708978, 40022254520559829, 4165689971, 13735928815302471318, 15174030263403373357, 2819236080141036144, 3879562584697143093, 5963323009994171033], absorbed := 3 }
  , cursorAfter := { stateWords := [7142307585975070536, 14398868398483481854, 10899121828257459710, 1354276691717841165, 3990289747763911550, 5911113892682090982, 11137158628670169364, 16176957937368330098], absorbed := 0 }
  , challengeOutput := (some 7142307585975070536)
  , digestOutput := none
}, {
  kind := .appendMessage
  ...
  , u64s := []
  , cursorBefore := { stateWords := [7142307585975070536, 14398868398483481854, 10899121828257459710, 1354276691717841165, 3990289747763911550, 5911113892682090982, 11137158628670169364, 16176957937368330098], absorbed := 0 }
  , cursorAfter := { stateWords := [19972606401193323, 39317353527350523, 1276472734, 9843709175946024893, 2729093451359831244, 7286394986216689390, 12079993863664365007, 11109941531923348717], absorbed := 3 }
  , challengeOutput := none
  , digestOutput := none
}, {
  kind := .appendMessage
  ...
  , u64s := []
  , cursorBefore := { stateWords := [19972606401193323, 39317353527350523, 1276472734, 9843709175946024893, 2729093451359831244, 7286394986216689390, 12079993863664365007, 11109941531923348717], absorbed := 3 }
  , cursorAfter := { stateWords := [54301200923914730, 32293443908449802, 587715080, 6467640991560515473, 18359844735620909772, 3345286153999714150, 4024687915221090244, 15881094469388127292], absorbed := 3 }
  , challengeOutput := none
  , digestOutput := none
}, {
  kind := .appendMessage
  ...
  , u64s := []
  , cursorBefore := { stateWords := [54301200923914730, 32293443908449802, 587715080, 6467640991560515473, 18359844735620909772, 3345286153999714150, 4024687915221090244, 15881094469388127292], absorbed := 3 }
  , cursorAfter := { stateWords := [372012156611796, 47292856697838762, 1591093796, 13246312632127660506, 680185555957310868, 15008659368874680188, 2308060211333043668, 9912676196090992961], absorbed := 3 }
  , challengeOutput := none
  , digestOutput := none
}, {
  kind := .challengeField
  ...
  , message := (bytes [])
  , u64s := []
  , cursorBefore := { stateWords := [372012156611796, 47292856697838762, 1591093796, 13246312632127660506, 680185555957310868, 15008659368874680188, 2308060211333043668, 9912676196090992961], absorbed := 3 }
  , cursorAfter := { stateWords := [4454742556977955767, 10561056814057217022, 7880073392391726247, 1048894732138297892, 10213127685364419419, 11412407036657020143, 9449916519192275243, 16123153056798045011], absorbed := 0 }
  , challengeOutput := (some 4454742556977955767)
  , digestOutput := none
}, {
  kind := .digest32
  , label := (bytes [])
  , message := (bytes [])
  , u64s := []
  , cursorBefore := { stateWords := [4454742556977955767, 10561056814057217022, 7880073392391726247, 1048894732138297892, 10213127685364419419, 11412407036657020143, 9449916519192275243, 16123153056798045011], absorbed := 0 }
  , cursorAfter := { stateWords := [3743543814592388887, 17352403964405870832, 10534696271804353061, 7949576757539374518, 13110359442851691325, 14401925026132298933, 2315074370510893254, 7692271446554784035], absorbed := 0 }
  , challengeOutput := none
  , digestOutput := (some (bytes [23, 223, 227, 229, 67, 189, 243, 51, 240, 240, 125, 249, 119, 31, 208, 240, 37, 202, 135, 169, 113, 194, 50, 146, 182, 5, 214, 59, 241, 145, 82, 110]))
}]
}
  , kernel := {
  ...
  , stage2Digest := (bytes [69, 185, 103, 181, 219, 7, 84, 178, 33, 50, 181, 108, 252, 31, 114, 167, 165, 28, 113, 61, 140, 213, 232, 206, 71, 7, 48, 142, 115, 98, 75, 248])
  ...
  , stage3ContinuityMix := 7142307585975070536
  , kernelFinalMix := 4454742556977955767
  , transcriptFinalDigest := (bytes [23, 223, 227, 229, 67, 189, 243, 51, 240, 240, 125, 249, 119, 31, 208, 240, 37, 202, 135, 169, 113, 194, 50, 146, 182, 5, 214, 59, 241, 145, 82, 110])
  , finalPc := 16
